...
class Func:
   ...
   def get_path_from_list(self,input_module):
      ...
      path_to_list = self.get_block_list()
   ...
   def no_com_verilog(self,input_file_p):   
      try:
   ...
   def fast_rm_com(self,input_file_p):
      ...
      return output_file
   ##-------------------------------------------------------------------------------------
   def get_inst_name(self,input_file_p):
      ...
      if result1 :
         ...
         port_dir , port_range = self.port_direction(module,port_name)
   ...
   ##-------------------------------------------------------------------------------------
   def port_direction(self,module,port_name):
     
      if 'last_module' in locals():
         if module != last_module :
            path_to_module = self.get_path_from_list(module)
            last_module = module
      else:
         path_to_module = self.get_path_from_list(module)
         last_module = module
   ...
   ##-------------------------------------------------------------------------------------
   def roots(self,input_path):
      #return path to uncomment file
      uncom_file = self.fast_rm_com(input_path)
      if uncom_file is None :
         return None
   
      this_module = self.module_name(uncom_file)
         ...
         with open(path_to_list,'r') as listfile:
            ...
            for i,line in enumerate(listfile):
               ...
               if commant_line == 'No' and os.path.exists(path_to_module):
                  uncom_file = self.fast_rm_com(path_to_module)
   ...
   def root(self,input_path):
      ...
      try:
         ...
         with open(path_to_list,'r') as listfile:      
            for i,line in enumerate(listfile):
               self.progress_bar("Searching:\ ",i+1,nol)
               path_to_module = line.rstrip('\n')
               commant_line = 'Yes' if ((len(path_to_module) > 1) and (path_to_module[0]+path_to_module[1] == "//")) else 'No'
   ...
   ##-------------------------------------------------------------------------------------
   def module_name(self,input_path):
      try:
   ...
   ##-------------------------------------------------------------------------------------
   def get_net_type(self,input_path,input_net):
      try:
         with open(input_path,'r') as file_in:
            text = file_in.read()
      except:
      ...
      regex = r'(?P<type>output|input|inout|wire|reg|genvar|integer|parameter|localparam)\s+(?P<range>\[[^:]*\:[^]]*\])?\s*'+input_net+'(\s*\=\s*(?P<defval>[^\s]*))?\s*\;'
      result = re.search(regex,text)
   ...
   ##-------------------------------------------------------------------------------------
   def progress_bar(self,string,index,max_index):
      if (index <= max_index):
